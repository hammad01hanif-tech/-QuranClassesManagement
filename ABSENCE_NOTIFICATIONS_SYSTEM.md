...
## نظرة عامة
نظام آلي لإشعار الإدارة عند وصول الطالب للغياب الثاني أو الثالث بدون عذر في الشهر الهجري الواحد، مع عرض أنيق ومنظم في قسم الإشعارات بصفحة الإدارة.
...
if (absenceCount === 2 || absenceCount === 3) {
  // إنشاء إشعار فقط للغياب الثاني والثالث
}
```
...
```javascript
// في checkAndNotifyAbsenceViolations()
// حالياً: الثاني والثالث فقط
if (absenceCount === 2 || absenceCount === 3) {
  // إنشاء إشعار
}

// لإضافة الرابع مثلاً:
if (absenceCount >= 2 && absenceCount <= 4) {
  // إنشاء إشعار
}
```
